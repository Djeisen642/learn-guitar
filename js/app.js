...
if ('serviceWorker' in navigator) {
  ...
  // time this page has already been served from the old cache — so without
  // this you'd see the previous version and only get the new one on a second
  // visit. Reload once when a new worker takes over.
  const hadController = !!navigator.serviceWorker.controller;
  const openedAt = Date.now();
  let reloading = false;

  navigator.serviceWorker.addEventListener('controllerchange', () => {
    // Not on first install — there was no old version to replace. And not
    // once someone is settled in and possibly mid-drill; the next visit will
    // pick it up anyway.
    if (!hadController || reloading || Date.now() - openedAt > 10_000) return;
    reloading = true;
    location.reload();
  });
